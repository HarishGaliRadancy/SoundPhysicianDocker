...
namespace SoundPhysicianDocker
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            // Get clientId and clientSecret from environment variables to avoid hardcoding
            string? clientId = Environment.GetEnvironmentVariable("CLIENT_ID");
            string? clientSecret = Environment.GetEnvironmentVariable("CLIENT_SECRET");

            if (string.IsNullOrEmpty(clientId) || string.IsNullOrEmpty(clientSecret))
            {
                logger.LogError("Client ID or Client Secret environment variables are not set.");
                return;
            }
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                if (!string.IsNullOrEmpty(jsonResponse))
                {
                    ...
                    string? xmlPath = config["XmlOutputPath"];
                    if (string.IsNullOrEmpty(xmlPath))
                    {
                        logger.LogError("XmlOutputPath is not configured in appsettings.json.");
                        return;
                    }
                ...
                {
                    logger.LogError("Job postings response was empty.");
                }
            }
            else
            {
